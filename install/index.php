    ...
    foreach ($_SESSION as $key => $value) {
        ...
        if ($key != 'usage' && empty($value)) {
            ...
            if ($key == 'server_path') {
                ...
                if ($key == 'mail_admin' && !Validator::email($value)) {
                    $errors[] = $locale['step_config_mail_admin_error'];
                    break;
                } else {
                    if ($key == 'mail_address' && !Validator::email($value)) {
                        $errors[] = $locale['step_config_mail_address_error'];
                        break;
                    } else {
                        if ($key == 'timezone' && !in_array($value, DateTimeZone::listIdentifiers())) {
                            $errors[] = $locale['step_config_timezone_error'];
                            break;
                        } else {
                            if ($key == 'client' && !in_array($value, $config['clients'])) {
            ...
            if (isset($_SESSION['var_account'])) {
                if (empty($_SESSION['var_account'])) {
                    ...
                    if (!Validator::accountName($_SESSION['var_account'])) {
                        ...
                        if (strtoupper($_SESSION['var_account']) == strtoupper($password)) {
                ...
                if (isset($_SESSION['var_account_id'])) {
                    if (empty($_SESSION['var_account_id'])) {
                        ...
                        if (!Validator::accountId($_SESSION['var_account_id'])) {
                            ...
                            if ($_SESSION['var_account_id'] == $password) {
