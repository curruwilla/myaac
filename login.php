...
/**
 * ENDPOINT - login.php
 * NECESSÁRIO PARA LOGIN NO TIBIA CLIENT.
 *
 * @author William Alvares <https://github.com/curruwilla/myaac>
 */
...
switch ($action) {
        ...
        foreach ($tableevent as $event) {
            if ($event) {
                $tmplist = [
                    ...
                    'displaypriority' => intval(parseEvent($event->getElementsByTagName('details'), false,
                        'displaypriority')),
                    'enddate' => intval(parseEvent($event, true, false)),
                    'isseasonal' => getBoolean(intval(parseEvent($event->getElementsByTagName('details'), false,
                        'isseasonal'))),
    ...
    case 'login':
        ...
        if ($query->rowCount() > 0) {
            $query = $query->fetch();
            $premDays = intval($query['premdays']);
            $lastDay = intval($query['lastday']);
            $lastLogin = $lastDay;
        } else {
