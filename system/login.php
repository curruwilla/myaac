...
if (ACTION === 'logout' && !isset($_REQUEST['account_login'])) {
    if (isset($account_logged) && $account_logged->isLoaded()) {
        if ($hooks->trigger(HOOK_LOGOUT, ['account' => $account_logged, 'password' => getSession('password')])) {
            unsetSession('account');
    ...
    if (!$logged && isset($_POST['account_login'], $_POST['password_login'])) {
        ...
        if (!empty($login_account) && !empty($login_password)) {
            ...
            if ($account_logged->isLoaded() && encrypt(($config_salt_enabled ? $account_logged->getCustomField('salt') : '') . $login_password) == $account_logged->getPassword()
                ...
                $hooks->trigger(HOOK_LOGIN, ['account' => $account_logged, 'password' => $login_password, 'remember_me' => $remember_me]);
            } else {
                $hooks->trigger(HOOK_LOGIN_ATTEMPT, ['account' => $login_account, 'password' => $login_password, 'remember_me' => $remember_me]);
